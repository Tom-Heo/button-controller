...
description: 버튼형 메시지로 부족한 정보를 확인하고 싶은 상황, 버튼형 메시지로 사용자에게 제안을 하고 싶은 상황, 사용자가 버튼을 누른 상황, 되돌리기 어려운 작업을 실행하기 전 승인을 받고자 하는 상황, 사용자의 현재 상황이나 맥락을 확인하고 싶은 상황에 사용하는 SKILL
---
# button-controller
...
### Triggers
- 버튼형 메시지로 부족한 정보를 확인하고 싶은 상황
- 버튼형 메시지로 사용자에게 제안을 하고 싶은 상황
- 사용자가 버튼을 누른 상황
...
5. 사용자가 버튼을 누르면 편집 기능을 사용하여 해당 메시지의 버튼을 제거한다.
...
{
  "action": "send",
  "message": "**[작업]**\n\n현재 시각 오전 9시 39분입니다.\n잠시 후 오전 10시에 ‘기획 회의’가 시작됩니다.\n\n11분 후인 오전 9시 50분에 알림을 추가해 드릴까요?",
  "buttons": [
    [
      { "text": "예", "callback_data": "button_yes", "style": "success" },
      { "text": "아니오", "callback_data": "button_no", "style": "primary" }
...
{
  "action": "send",
  "message": "**[작업]**\n\n토요일 오후부터 서울 지역에 강수 예보가 있습니다.\n‘신사 가로수길 데이트’에 차질이 생길 수 있습니다.\n\n기상 예보가 바뀔 수 있으니 데이트 전 날에 해당 내용을 다시 알려드릴까요?",
  "buttons": [
    [
      { "text": "예", "callback_data": "button_yes", "style": "success" },
      { "text": "아니오", "callback_data": "button_no", "style": "primary" }
...
{
  "action": "send",
  "message": "**[제안]**\n\n내일 오후 5시에 ‘기초통계학 과제’가 마감됩니다.\n마감 기한까지 과제 수행 및 제출 일정이 없는 것으로 확인됩니다.\n\n오늘 저녁 9시에 ’기초통계학 과제 수행 및 제출‘ 일정을 추가할까요?",
  "buttons": [
    [
      { "text": "예", "callback_data": "button_yes", "style": "success" },
      { "text": "아니오", "callback_data": "button_no", "style": "primary" }
...
{
  "action": "send",
  "message": "**[작업]**\n\n내일 오후 5시에 ‘기초통계학 과제’가 마감됩니다.\n마감 기한까지 과제 수행 및 제출 일정이 없는 것으로 확인됩니다.\n\n캘린더 내 해당 할 일의 비고란에 ‘미제출’ 상태를 기록할까요?",
  "buttons": [
    [
      { "text": "예", "callback_data": "button_yes", "style": "success" },
      { "text": "아니오", "callback_data": "button_no", "style": "primary" }
...
{
  "action": "send",
  "message": "**[제안]**\n\n‘조형준 교수님 면담’ 일정 추가를 내일 오전 11시로 요청하셨습니다.\n해당 시간은 기존 루틴인 ‘통계수학’의 강의 시간(09:00-11:45)과 겹칩니다.\n\n‘통계수학’ 담당 교수님께 전송드릴 불참 사유 메일 작성을 도와드릴까요?",
  "buttons": [
    [
      { "text": "예", "callback_data": "button_yes", "style": "success" },
      { "text": "아니오", "callback_data": "button_no", "style": "primary" }
...
- 사용자가 버튼을 클릭하면 해당 버튼형 메시지의 버튼은 삭제한다.
- 버튼형 메시지를 남용하지 않는다.
...
- 사용자가 버튼을 눌렀음에도 해당 메시지의 버튼을 제거하지 않는 실수. 사용자가 버튼을 누르면 해당 메시지의 버튼은 제거해야 한다.
- 버튼형 메시지의 버튼만 제거해야 하는데 메시지 자체를 삭제하는 실수. 사용자가 버튼을 클릭하면 Telegram의 편집 기능(action: edit)으로 해당 메시지의 버튼만 제거해야 한다.
...
- 본문에 헤더 볼드 외의 불필요한 마크다운 서식을 사용하는 실수. 볼드 처리는 헤더에만 적용하고, 본문은 순수 텍스트로 작성해야 한다.
...
- 사용자가 보낸 메시지를 그대로 인용하여 상황 및 맥락에 넣는 실수. 상황 및 맥락은 시스템이 파악한 사실을 정리한 문장이어야 한다.
- 버튼형 메시지 없이 일반 텍스트로 선택지를 나열하는 실수. 사용자에게 선택지를 제시할 때는 반드시 버튼형 메시지를 사용해야 한다.
...
- 버튼형 메시지를 보낸 뒤 시스템 점검이나 세션 초기화로 이전 메시지의 message_id를 참조할 수 없는 경우, 버튼 제거를 시도하지 않고 새 메시지로 상황을 이어간다.